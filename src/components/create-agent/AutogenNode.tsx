...
const getTeamTypeInfo = (teamType?: string) => {
  if (!teamType) return { icon: Users, color: 'text-gray-500', bgColor: 'bg-gray-100', label: 'Team', description: 'AI agents team' };

  const type = teamType.toLowerCase();
  if (type.includes('roundrobin')) return { icon: RotateCcw, color: 'text-blue-600', bgColor: 'bg-blue-50', label: 'Round Robin', description: 'Agents take turns in sequence' };
  if (type.includes('selector')) return { icon: Target, color: 'text-purple-600', bgColor: 'bg-purple-50', label: 'Selector', description: 'LLM selects next speaker dynamically' };
  if (type.includes('magneticone') || type.includes('magenticone')) return { icon: Zap, color: 'text-yellow-600', bgColor: 'bg-yellow-50', label: 'Magnetic One', description: 'Generalist multi-agent for web/file tasks' };
  if (type.includes('swarm')) return { icon: Network, color: 'text-green-600', bgColor: 'bg-green-50', label: 'Swarm', description: 'HandoffMessage for explicit transitions' };
  if (type.includes('graphflow') || type.includes('graph')) return { icon: GitBranch, color: 'text-indigo-600', bgColor: 'bg-indigo-50', label: 'Graph Flow', description: 'Complex workflows with branches & loops' };
  if (type.includes('hierarchical')) return { icon: GitBranch, color: 'text-purple-600', bgColor: 'bg-purple-50', label: 'Hierarchical', description: 'Manager delegates to subordinates' };
  if (type.includes('cascading')) return { icon: ArrowRight, color: 'text-orange-600', bgColor: 'bg-orange-50', label: 'Cascading', description: 'Try agents in sequence until success' };

  return { icon: Users, color: 'text-gray-600', bgColor: 'bg-gray-50', label: 'Custom', description: 'Custom team configuration' };
};
  ...
  if (data.type === 'agent') {
    return (
      <>
          ...
          <div className="p-1.5 border-b border-gray-100">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={handleNodeEdit} className="text-gray-400 hover:text-gray-600">
        ...
        {/* 🚀 Updated to Left (input) and Right (output) */}
        <Handle 
          type="target" 
  ...
  return (
    <>
      <div className="bg-white rounded-md border border-gray-200 w-[240px]">
        <div className="p-2 border-b border-gray-100">
          ...
          <div className={`flex items-center gap-2 px-2 py-1.5 rounded-md ${teamTypeInfo.bgColor} mb-2`}>
            ...
            <div className="flex-1">
              <span className={`text-[10px] font-semibold ${teamTypeInfo.color} block`}>{teamTypeInfo.label}</span>
              <span className="text-[8px] text-gray-600 leading-tight">{teamTypeInfo.description}</span>
            </div>
          </div>
        ...
        {data.description && (
          <div className="p-2 text-[9px] text-gray-600 border-b border-gray-100">{data.description}</div>
        )}
        ...
        <div className="px-2 py-1.5 border-t border-gray-100">
          <div className="text-[9px] font-medium text-gray-500 uppercase">TERMINATIONS</div>
